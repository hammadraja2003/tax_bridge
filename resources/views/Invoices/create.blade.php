    ...
    <div class="item-group border rounded p-3 mb-4 position-relative">
      ...
      <div class="row g-3">
        ...
        <div class="col-md-4">
          ...
          <input type="number" min="0" step="0.01" name="items[][valueSalesExcludingST]" class="form-control"
            required />
        </div>
        <div class="col-md-4">
    ...
    document.addEventListener("DOMContentLoaded", function () {
          ...
          setTimeout(() => {
              existingItems.forEach((item, index) => {
                  ...
                  $row.find('[name$="[productDescription]"]').val(item.product_description);
                  ...
                  $row.find('[name$="[valueSalesExcludingST]"]').val(item.value_sales_excluding_st);
                  $row.find('[name$="[totalValues]"]').val(item.total_values);
                  $row.find('[name$="[fixedNotifiedValueOrRetailPrice]"]').val(item.fixed_notified_value_or_retail_price);
                  $row.find('[name$="[SalesTaxApplicable]"]').val(item.sales_tax_applicable);
                  $row.find('[name$="[SalesTaxWithheldAtSource]"]').val(item.sales_tax_withheld_at_source);
                  ...
                  $row.find('[name$="[sroItemSerialNo]"]').val(item.sro_item_serial_no);
              });
