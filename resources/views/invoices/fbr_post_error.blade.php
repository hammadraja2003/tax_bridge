                        ...
                        <div class="app-scroll overflow-auto">
                            <table class="table table-striped table-bordered m-0">
                                ...
                                <tbody>
                                    @forelse($fbr_errors as $err)
                                        <tr>
                                            <td>{{ \Carbon\Carbon::parse($err->error_time)->format('d-M-Y H:i:s') }}</td>
                                            <td>{{ $err->type }}</td>
                                            <td>{{ $err->fbr_env }}</td>
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="9" class="text-center text-muted">No FBR posting errors found.
                                            </td>
                                        </tr>
                                    @endforelse
                                </tbody>
                            ...
                            <div class="d-flex justify-content-between align-items-center px-3 py-2 small text-muted">
                                <div id="table-count-info">
                                    Showing {{ $fbr_errors->firstItem() ?? 0 }} to {{ $fbr_errors->lastItem() ?? 0 }} of
                                    {{ $fbr_errors->total() }} entries
                                </div>
                                <div class="list-pagination">{!! $fbr_errors->links() !!}</div>
                            </div>
                        </div>
                    </div>
                </div>
